...
\end{align}
where $V_{x,y}$ swaps the spin $x$ and $y$; $V|i,j\rangle = |j,i\rangle$.
Then, for an $\hat{O}$ acting on site 0, $U_{sw}$ moves $\hat{O}$ $2n$ sites to the left and $U_{sw}^{-n}$ moves $\hat{O}$ $2n$ sites to the right
and thus the condition is satisfied.
In this case, we can write every step analytically and $1/M^2$ scaling and the cosine modulation follows for any $M$.
In a generic quantum circuit, however, these features are seen for sufficiently large $M$. 
