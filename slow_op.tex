...
\begin{figure}
\includegraphics[width=1.0\linewidth]{random_circuits.pdf}
\centering
\caption{ (color online) (a) $\lambda$ as a function of $M$ for a random circuit model. Each $\lambda$ is averaged over 50 realizations.
$\lambda$ also decreases in this model.
 (b) $\tilde{\lambda}$ as a function of $N$ (operator is supported on $M$ = $4N+1$ sites) for a random circuit model computed by matrix product method with assuming a specific form of operators as Eq. \eqref{eq:filter}. We averaged by 50 realizations of random circuits. The data well converged at the bond dimension $D = 80$.
Inset: Structure of optimal $c_n$'s of randomly chosen three realizations for each $N$ = 10 (blue), 40 (red), and 100 (black). Each case the largest magnitude $c_n$ is normalized to one. Within the same $N$, $c_n$'s of three realizations are almost indistinguishable. }
\label{fig:random_circuit}
\end{figure}

To extend to larger systems, we use a matrix product method and assume a specific form of the optimal operator.  We consider operators in the space spanned by $U^n \hat{O} U^{\dag n}$ for $\hat{O}$ a traceless hermitian operator acting on a single site and taking a finite number of powers of $U$.
We find the optimal operator of the form,
\begin{align}
 \tilde{A}_N=\sum_{n=-N}^N c_n U^n \hat{O} U^{\dag n} ~,
 \label{eq:filter}
\end{align}
that minimize $\tilde{\lambda} = \mathrm{tr}([\tilde{A}_N,U][\tilde{A}_N,U]^\dag)/\mathrm{tr}(\tilde{A}_N\tilde{A}_N^\dag)$.
...
Note that each $U^n \hat{O} U^{\dag n} + U^{\dag n} \hat{O} U^n$ is supported on $4n+1$ sites so $\tilde{A}_N$ has the range $M = 4N+1$.
We choose $\hat{O} = 1/2\sigma^z$ as a single site hermitian.
The results are shown in Figure \ref{fig:random_circuit} (b).
The weights $c_n$ approximately follow a cosine shape and $\lambda$ decays roughly as $1/M^2$.
Such a decay $1/M^2$ would be exact if
${\rm tr}(\hat{O}^\dagger U^n \hat{O} U^{\dag n})=0$ for all $n \neq 0$.
