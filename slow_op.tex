...
\centering
\caption{(color online) (a) $\lambda(M)$ vs. $M$.  As we increase $M$, the commutator with the Hamiltonian decreases as shown.
...
We consider local operators supported on a finite interval of consecutive $M$ sites. %; an operator supported on consecutive $M$ sites is said to have ``range $M$".
...
$a_M^{th} \simeq \rm{tr}(\hat{A}_M) = 0$ with negligible error.
...
\end{align}
$f(\hat{A}_M)$ characterizes the thermalization time scale $\tau$ of $\hat{A}_M$ by $\tau \sim f(\hat{A}_M)^{-1/2}$. 
...
It is clear that $\lambda(M)$ decreases with $M$.
...
Figure \ref{fig:hamiltonian} (b) plots the minimum of $f(\hat{A}_M)$ in this restricted space.
...
\end{align}
Again, we define $\lambda(M) = \mathrm{min}\{g(\hat{A}_M)\}$. 
...
\end{align}
This proves that $g(\hat{A}_M)$ characterizes the thermalization time step $\tau$ of $\hat{A}_M$ by
$\tau \sim g(\hat{A}_M) ^{-1/2}$. Again, small $\lambda(M)$ implies a long thermalization time step. 
