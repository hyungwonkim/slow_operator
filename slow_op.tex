...
matches the expectations from diffusive hydrodynamics, with the square of the Frobenius norm of the commutator $\sim M^{-2}$ for the slowest such operator on a subregion of the length $M$ spins.
...
$a_M^{th} = \rm{tr}(\hat{A}_M) = 0$ with negligible error.
