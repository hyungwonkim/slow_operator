...
with the Hamiltonian, and thus is ``almost conserved''.  We find slow operators with significantly slower relaxation than
...
Slow operators are present in any such system, so long as the unitary dynamics is local.  Locality restricts an interval of length $M$ from fully relaxing on a time scale faster than $\sim M$, so these slow operators have to exist
%They themselves do not inhibit relaxation of the local density matrices ``as fast as possible" (i.e., on a time scale proportional to the length of the interval;
...
%This study was performed using a Lanczos method.
