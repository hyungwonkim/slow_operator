...
$\lambda$ also decreases in this model.  
 (b) $\tilde{\lambda}$ as a function of $N$ (operator is supported on $M$ = $4N$ sites) for a random circuit model computed by matrix product method with assuming a specific form of operators as Eq. \eqref{eq:filter}. We averaged by 50 realizations of random circuits. The data well converged at the bond dimension $D = 80$.
...
To extend to larger systems, we use a matrix product method.  In this case, we consider operators in the space spanned by $U^n \hat{O} U^{\dag n}$ for $\hat{O}$ a traceless hermitian operator acting on a single site and taking a finite number of powers of $U$. 
...
Note that each $U^n \hat{O} U^{-n}$ is supported on $4n$ sites so $\tilde{A}_N$ has the range $M = 4N$. 
